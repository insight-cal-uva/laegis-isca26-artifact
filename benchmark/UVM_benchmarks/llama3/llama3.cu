...
void malloc_run_state(RunState *s, Config *p) {
    ...
    CUDA_CHECK(cudaMallocManaged((void **) &s->x, p->dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->xb, p->dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->xb2, p->dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->hb, p->hidden_dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->hb2, p->hidden_dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->q, p->dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->key_cache, p->n_layers * p->max_seq_len * kv_dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->value_cache, p->n_layers * p->max_seq_len * kv_dim * sizeof(float)));
    CUDA_CHECK(cudaMallocManaged((void **) &s->att, p->n_heads * p->max_seq_len * sizeof(float)));    
    CUDA_CHECK(cudaMallocManaged((void **) &s->logits_gpu, p->vocab_size * sizeof(float)));
...
void read_checkpoint(char *checkpoint, Config *config, TransformerWeights *weights,
    ...
    *data = (float *) mmap(NULL, *file_size, PROT_READ, MAP_PRIVATE, *fd, 0);
    ...
    CUDA_CHECK(cudaMallocManaged((void **) &weights_ptr, weights_size));
    cudaDeviceSynchronize();
    // CUDA_CHECK(cudaMemcpy(weights_ptr, *data + sizeof(Config) / sizeof(float), weights_size, cudaMemcpyHostToDevice));
    memcpy(weights_ptr, *data + sizeof(Config) / sizeof(float), weights_size);
...
float *forward(Transformer *transformer, int token, int pos) {
    ...
    cudaDeviceSynchronize();
    memcpy(s->logits, s->logits_gpu, p->vocab_size * sizeof(float));
    return s->logits;
}
...
int main(int argc, char *argv[]) {
    ...
    int max_new_tokens = 1024;                            // number of max_new_tokens to run for
    char *prompt = (char *) "In have a dream,";      
