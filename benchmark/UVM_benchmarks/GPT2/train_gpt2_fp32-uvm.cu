...
float* malloc_and_point_parameters(ParameterTensors* params, size_t* param_sizes, int on_device) {
    ...
    if (on_device) {
        // yy: change to uvm
        cudaCheck(cudaMallocManaged((void**)&params_memory, num_parameters * sizeof(float)));
    } else {
...
typedef struct {
    ...
    int* inputs; // the input tokens for the current forward pass     // yy: via uvm
    int* targets; // the target tokens for the current forward pass     // yy: via uvm
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path) {
    ...
    // yy: uvm
    memcpy(model->params_memory, params_memory_cpu, num_parameters * sizeof(float));
...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, int B, int T) {
    ...
    if(model->acts_memory == NULL) {
        ...
        // yy: change to uvm
        cudaCheck(cudaMallocManaged((void**)&model->inputs, B * T * sizeof(int)));
        // yy: change to uvm
        cudaCheck(cudaMallocManaged((void**)&model->targets, B * T * sizeof(int)));
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, int t) {
    ...
    if (model->m_memory == NULL) {
        // yy: change to uvm
        cudaCheck(cudaMallocManaged((void**)&model->m_memory, model->num_parameters * sizeof(float)));
        cudaCheck(cudaMallocManaged((void**)&model->v_memory, model->num_parameters * sizeof(float)));
    ...
    for (int step = 0; step <= train_num_batches; step++) {
        ...
        if (step > 0 && step % sample_every == 0 || last_step) {
            ...
            for (int t = 1; t < genT; t++) {
                ...
                // yy: uvm
                memcpy(cpu_logits, logits, model.config.vocab_size * sizeof(float));
