...
public:
    ...
    // Adapte to the new ramulator
    GDDR5(Org org, Speed speed, int channel_width, int prefetch_size);
    GDDR5(const string& org_str, const string& speed_str, int channel_width, int prefetch_size);
    ...
    struct OrgEntry {
        ...
        // ba banks in a bank group
        // bg bank groups
        // size dq  ?   ch  bg ba   row    col
        {  512, 16, {0, 1, 4, 2, 1<<12, 1<<(7+3)}}, 
        {  512, 32, {0, 1, 4, 2, 1<<12, 1<<(6+3)}},
        {1<<10, 16, {0, 1, 4, 4, 1<<12, 1<<(7+3)}}, 
        {1<<10, 32, {0, 1, 4, 4, 1<<12, 1<<(6+3)}},
        {2<<10, 16, {0, 1, 4, 4, 1<<13, 1<<(7+3)}}, 
        {2<<10, 32, {0, 1, 4, 4, 1<<13, 1<<(6+3)}},
        {4<<10, 16, {0, 1, 4, 4, 1<<14, 1<<(7+3)}}, 
        {2<<10, 32, {0, 1, 4, 4, 1<<14, 1<<(6+3)}},
        {8<<10, 16, {0, 1, 4, 4, 1<<14, 1<<(8+3)}}, 
        {8<<10, 32, {0, 1, 4, 4, 1<<14, 1<<(7+3)}}
    }, org_entry;
    ...
    int channel_width = 32; // should be 32-bit 
