...
def draw_fig9_per_app(app_name):
    result = get_results_file(app_name,result_txt[6],commit_id)
    df = pd.read_csv(result[0])
    columns_of_interest = ["n_read","n_write"]
    ...
    plt.title(f'Page Read Write Counts by Aggregated {large_pfn} 4KB Pages')
    ax.set_xticklabels([''] * len(df_aggregated.index), rotation=45, ha='right')
    plt.legend(ncols=1)

    # Show the plot
    plt.tight_layout()
    figure_path = root_dir+"/saved_figures/"+"fig9-ori-"+app_name+".png"
    plt.savefig(figure_path,dpi=1200)
    plt.close()



draw_fig9_per_app(uvm_dirs[0])
draw_fig9_per_app(uvm_dirs[1])