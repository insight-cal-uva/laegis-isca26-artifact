...
private:
  ...
  void refresh_target(Controller<T>* ctrl, int rank, int bank, int sa)
  {
    ...
    addr_vec[2] = bank;
    addr_vec[3] = sa;
    Request req(addr_vec, request::Type::REFRESH, NULL);//Request::Type::REFRESH
...
// template<> Refresh<HMS>::Refresh(Controller<HMS>* ctrl);
// template<> void Refresh<HMS>::tick_ref();
// template<> void Refresh<HMS>::inject_refresh(bool b_ref_rank);
