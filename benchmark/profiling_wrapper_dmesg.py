...
dnn_folders = ['CNN', 'ResNet',]
new_apps =[]

folders = ["BN","CNN" ,"knn", "logistic-regression", "rodinia", "polybench"]
...
all_apps = ["BN","CNN", "knn", "logistic-regression", 
...
config_names = ['default','prefetch', 'coalesce', 'batch_count', 'batch_per_serv', 'access_ctr', 'prefetch_ctr', 'prefetch_minf']

dmesg_config1 = ['default', 'p1', 'p21', 'p51', 'p71', 'p91', 'p100']
dmesg_config1 = ['default', 'b1024']
...
top_dir = "/asplos/uvm_bench/dmesg_results"

# store the result at which level
def profile(config_name: str="p71b256", postfix: str=None):
    ...
    for UVM_flag in [True]:
        ...
        if UVM_flag:
            base_path = "/asplos/uvm_bench/UVM_benchmarks/"
        else:
            base_path = "/asplos/uvm_bench/non_UVM_benchmarks/"
        ...
        for i, rfolder in enumerate(folders):
            ...
            if rfolder != "rodinia" and rfolder != "polybench":
                ...
                if UVM_flag:   
                    command1 = "cd " + path + "; dmesg -C ;" + " ./run"
                    command2 = "dmesg > " + rfolder+"-"+config+ ".txt"
                    command3 = "mv " + rfolder+"-"+config+ ".txt " + output_path
                    all_cmds.append([command1,command2, command3])
                ...
                for j, rsubfolder in enumerate(subfolders2):
                    ...
                    if UVM_flag:
                        command1 = "cd " + path + "; dmesg -C ;" + " ./run"
                        command2 = "dmesg > " + rsubfolder+"-"+config + ".txt"      
                        command3 = "mv "+ rsubfolder+"-"+config+ ".txt " + output_path     
                        all_cmds.append([command1,command2, command3])
