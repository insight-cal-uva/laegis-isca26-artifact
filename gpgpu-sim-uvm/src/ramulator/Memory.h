...
public:
    MemoryBase() {}
    virtual ~MemoryBase() {}
    virtual double clk_ns(double dram_freq) = 0;
    virtual void tick() = 0;
    virtual bool send(Request& req) = 0;
    virtual void send_pcie(Request& req) = 0;
    virtual int pending_requests() = 0;
    virtual void finish(unsigned kernel_id) = 0;
...
protected:
  ...
  // used for HMS
  long scm_max_address;
  long dram_max_address;
...
public:
    enum class Type {
        ChRaBaRoCo,
        RoBaRaCoCh,
        GPGPUSIM, // use address mapping generated by GPGPU-SIM.
        MAX,
    } type = Type::GPGPUSIM;
    ...
    // used for HMS
    vector<int> dram_addr_bits;
    vector<int> scm_addr_bits;
    string mapping_file;
    ...
    bool full(bool is_write) {
      request::Type type = (is_write) ? request::Type::R_WRITE : request::Type::R_READ;////Request::Type
      return ctrls[0]->full(type);
    }

    double clk_ns(double dram_freq)
    {   
        std::cout <<" speed_freq: "<< spec->speed_entry.freq << std::endl;
        std::cout <<" dram_freq: "<< dram_freq << std::endl;
        assert(spec->speed_entry.freq == dram_freq && 
               "Make sure that you correctly configure the memory clock frequency");
    ...
    bool send(Request& req) {
      ...
      req.addr_vec.resize(int(T::Level::MAX));
      assert(req.mf->get_tlx_addr().chip == ctrls[0]->ch_id &&
             "request arrives at the wrong memory partition");
      req.addr_vec[int(T::Level::Channel)] = 0;
      req.addr_vec[int(T::Level::BankGroup)] = req.mf->get_tlx_addr().bk & (4 - 1);
      req.addr_vec[int(T::Level::Bank)] = req.mf->get_tlx_addr().bk >> 2;
      assert(req.addr_vec[int(T::Level::BankGroup)] < 4 &&
             req.addr_vec[int(T::Level::Bank)] < 4);
      req.addr_vec[int(T::Level::Rank)] = req.mf->get_tlx_addr().rank;
    
      req.addr_vec[int(T::Level::Row)] = req.mf->get_tlx_addr().row;
      // GPGPU-sim generates column address including 5 LSB
      req.addr_vec[int(T::Level::Column)] = req.mf->get_tlx_addr().col >> 5;
    ...
    void send_pcie(Request& req) {
      ...
      req.addr_vec.resize(int(T::Level::MAX));
      req.addr_vec[int(T::Level::Channel)] = 0;
      req.addr_vec[int(T::Level::BankGroup)] = raw_addr.bk & (4 - 1);
      req.addr_vec[int(T::Level::Bank)] = raw_addr.bk >> 2;
      assert(req.addr_vec[int(T::Level::BankGroup)] < 4 &&
             req.addr_vec[int(T::Level::Bank)] < 4);
      req.addr_vec[int(T::Level::Rank)] = raw_addr.rank;
      
      req.addr_vec[int(T::Level::Row)] = raw_addr.row;
      // GPGPU-sim generates column address including 5 LSB
      req.addr_vec[int(T::Level::Column)] = raw_addr.col >> 5;
    ...
    // called from Ramulator::finish()
    void finish(unsigned kernel_id) {
      // 1024*1024*1024
      dram_capacity = max_address;
      int *sz = spec->org_entry.count; // pointer to the array [ch,bg,bk,ra,co]
      ...
      for (auto ctrl : ctrls) { // seems only 1 ctrl per chip
        long read_req = 0;
        // call Controller.h: finsih()
...
//                                 const memory_config* m_config);
// template<>
// bool Memory<HMS, Controller>::send(Request&);
// template<>
// void Memory<HMS, Controller>::send_pcie(Request&);
// template<>
