...
Layer::Layer(int M, int N, int O) {
  ...
  bt_malloc = (double)(malloc_e - malloc_s) / CLOCKS_PER_SEC;
...
void Layer::cudafree() {
  ...
  clock_t free_e = clock();
	bt_free = (double)(free_e - free_s) / CLOCKS_PER_SEC;
}

// Send data one row from dataset to the GPU
void Layer::setOutput(float *data) {
  // cudaMemcpy(output, data, sizeof(float) * O, cudaMemcpyHostToDevice);
	clock_t memcpy_h2d_s = clock();
  memcpy(output, data, sizeof(float) * O); //TODO May change to prefetch
  cudaMemPrefetchAsync(output,sizeof(float) * O,  0, stream );
	clock_t memcpy_h2d_e = clock();
	bt_memcpy_h2d += (double)(memcpy_h2d_e - memcpy_h2d_s) / CLOCKS_PER_SEC;
...
void Layer::clear() {
  ...
  cudaMemset(preact, 0x00, sizeof(float) * O);
	...
	bt_memset += (double)(memset_e - memset_s) / CLOCKS_PER_SEC;
...
__global__ void makeError(float *err, float *output, unsigned int Y,
                          const int N) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
...
__global__ void fp_preact_c1(float input[28][28], float preact[6][24][24],
                             float weight[6][5][5]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
  ...
  for (int n = N * pos / size; n < N * (pos + 1) / size; ++n) {
    ...
    atomicAdd(&preact[i3][i4][i5],
              weight[i3][i1][i2] * input[i4 + i1][i5 + i2]);
  }
}
...
__global__ void fp_preact_s1(float input[6][24][24], float preact[6][6][6],
                             float weight[1][4][4]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
  ...
  for (int n = N * pos / size; n < N * (pos + 1) / size; ++n) {
    ...
    atomicAdd(&preact[i3][i4][i5],
              weight[0][i1][i2] * input[i3][i4 * 4 + i1][i5 * 4 + i2]);
  }
}
...
__global__ void fp_preact_f(float input[6][6][6], float preact[10],
...
__global__ void bp_weight_f(float d_weight[10][6][6][6], float d_preact[10],
                            float p_output[6][6][6]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
...
__global__ void bp_output_s1(float d_output[6][6][6],
                             float n_weight[10][6][6][6], float nd_preact[10]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
...
__global__ void bp_preact_s1(float d_preact[6][6][6], float d_output[6][6][6],
                             float preact[6][6][6]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
...
__global__ void bp_weight_s1(float d_weight[1][4][4], float d_preact[6][6][6],
                             float p_output[6][24][24]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
  ...
  for (int n = N * pos / size; n < N * (pos + 1) / size; ++n) {
    ...
    atomicAdd(&d_weight[i1][i2][i3],
              d_preact[i4][i5][i6] * p_output[i4][i5 * 4 + i2][i6 * 4 + i3]);
  }
}
...
__global__ void bp_output_c1(float d_output[6][24][24], float n_weight[1][4][4],
                             float nd_preact[6][6][6]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
  ...
  for (int n = N * pos / size; n < N * (pos + 1) / size; ++n) {
    ...
    atomicAdd(&d_output[i4][i5 * 4 + i2][i6 * 4 + i3],
              n_weight[i1][i2][i3] * nd_preact[i4][i5][i6]);
  }
}

__global__ void bp_preact_c1(float d_preact[6][24][24],
...
__global__ void bp_weight_c1(float d_weight[6][5][5], float d_preact[6][24][24],
                             float p_output[28][28]) {
  const int pos = blockIdx.x * blockDim.x + threadIdx.x;
  const int size = blockDim.x * gridDim.x;
  ...
  for (int n = N * pos / size; n < N * (pos + 1) / size; ++n) {
    ...
    atomicAdd(&d_weight[i1][i2][i3],
              d_preact[i1][i4][i5] * p_output[i4 + i2][i5 + i3] / d);
  }
}
