...
int main() {
  ...
  */
  ...
  // FILE * fp = fopen("/shared/uvm_bench/log/bn.txt", "a");
  // if (fp == NULL) {
...
void genScore() {
  ...
  // float t_malloc1 = (float)(malloc_e - malloc_s) / CLOCKS_PER_SEC;
...
float findBestGraph(float & kern_time) {
  ...
  for (node = 0; node < NODE_N; node++) {
    ...
    if (posN >= 0) {
      ...
      clock_t memcpy_s = clock();
      cudaMemcpy(D_parent, orders[node], NODE_N * sizeof(bool),
                 cudaMemcpyHostToDevice);
      clock_t memcpy_e = clock();
      bt_memcpy_h2d += (float)(memcpy_e - memcpy_s) / CLOCKS_PER_SEC;
      ...
      // printf("computeKernel time: %f sec\n", (float)(finish - start) / CLOCKS_PER_SEC);
      clock_t memcpy2_s = clock();
      cudaMemcpy(parents, D_resP, blocknum * 4 * sizeof(int),
                 cudaMemcpyDeviceToHost);
      cudaMemcpy(scores, D_Score, blocknum * sizeof(float),
                 cudaMemcpyDeviceToHost);
      clock_t memcpy2_e = clock();
      bt_memcpy_d2h += (float)(memcpy2_e - memcpy2_s) / CLOCKS_PER_SEC;
