...
public:
    ...
    // Constructor
    DRAM(T* spec, typename T::Level level, DRAM<T>* parent, int id, typename T::Type parent_type);
    ~DRAM();
    ...
    long size;
    DRAM* parent;
    vector<DRAM*> children; // Channel->Rank->Bank they all appear as DRAM<T>
...
void DRAM<T>::finish(long dram_cycles) {
  ...
  // (e.g., Channel (0)-> Rank (1)->Bank Group (2)->Bank (3)
  for (auto child : children) {
...
template <typename T>
DRAM<T>::DRAM(  T* spec, 
                typename T::Level level,  // T::Level::Channel = 0
                DRAM<T>* parent, // NULL
                int id,  // ch
                typename T::Type parent_type // T::Type::SINGLE
            ) :spec(spec), level(level), id(id), parent(parent)
{
    ...
    int child_level = int(level) + 1; // child_level = 1 at the beginning; (e.g., Channel (0) -> Rank (1) ->Bank Group->Bank->Row (4) ->Column)
    if (child_level == int(T::Level::Row))
        return; // stop recursion: rows are not instantiated as nodes
    // how many ranks, bank groups, banks
    ...
    for (int i = 0; i < child_max; i++) {
        DRAM<T>* child = 
          new DRAM<T>(spec, typename T::Level(child_level), this, i, type);
          //new DRAM<T>(spec, typename T::Level(child_level));
        //child->parent = this;
        //child->id = i;
        children.push_back(child);
    }
    
    // the dram level until row level
    if (g_debug_execution >= 3){
        std::cout << "DRAM children: "<< children.size() << std::endl;
    }
}
