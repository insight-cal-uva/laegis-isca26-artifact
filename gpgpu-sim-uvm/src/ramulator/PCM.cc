...
string PCM::level_str [int(Level::MAX)] = {"Ch", "Ra", "Bg", "Ba", "Ro", "Co"};

map<string, enum PCM::Org> PCM::org_map = {
  {"PCM_SLC", PCM::Org::PCM_SLC}, 
  {"PCM_MLC", PCM::Org::PCM_MLC}, 
  {"PCM_TLC", PCM::Org::PCM_TLC},
  {"VRRAM", PCM::Org::VRRAM}
};

map<string, enum PCM::Speed> PCM::speed_map = {
  {"PCM_SLC", PCM::Speed::PCM_SLC},
  {"PCM_MLC", PCM::Speed::PCM_MLC},
  {"PCM_TLC", PCM::Speed::PCM_TLC},
  {"VRRAM", PCM::Speed::VRRAM},
  {"PCM_SLC_A100", PCM::Speed::PCM_SLC_A100},
  {"PCM_MLC_A100", PCM::Speed::PCM_MLC_A100},
  {"PCM_TLC_A100", PCM::Speed::PCM_TLC_A100},
  {"VRRAM_A100", PCM::Speed::VRRAM_A100}
};


PCM::PCM(Org org, Speed speed, int channel_width, int prefetch_size)
    : org_entry(org_table[int(org)]),
      addr_mask(addr_mask_list[int(org)]),
...
{
    assert(channel_width == 128 && "The size of data bus for HBM-PCM must be 128 bits");
    assert(prefetch_size == 2 && "The prefetch size for HBM-PCM must be 2");
    init_speed();
    init_prereq();
