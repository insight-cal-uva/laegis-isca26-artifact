...
// template<>
// Refresh<HMS>::Refresh(Controller<HMS>* ctrl) : ctrl(ctrl) {
//   clk = refreshed = 0;
//   // HMS does not have to consider SCM
//   max_rank_count = 1;
//   max_bank_count = ctrl->channel->spec->dram_org_entry.count[(int)HMS::Level::Bank];
...
//   level_chan = (int)HMS::Level::Channel;
//   level_rank = (int)HMS::Level::Rank;
//   level_bank = (int)HMS::Level::Bank;
//   level_sa = -1;
// }

// template<>
// void Refresh<HMS>::tick_ref() {
//   clk++;
//   // will refresh DRAM, not PCM
//   int refresh_interval = ctrl->channel->spec->dram_speed_entry.nREFI;
...
// template<>
// void Refresh<HMS>::inject_refresh(bool b_ref_rank) {
//   if (b_ref_rank) {
//     // only inject refresh into DRAM
//     refresh_target(ctrl, HMS::DRAM_RANK_ID, -1, -1);
//   } else {
//     // Bank-level refresh. Simultaneously issue to all ranks (better performance than staggered refreshes).
//     assert(false && "HMS refresh error");
//   }
