...
class StatList {
 protected:
  ...
  std::vector<std::vector<StatBase*>> stat_list_per_channel;
 public:
  void add(StatBase* stat) {
    list.push_back(stat);
  }
  void add(StatBase* stat, int channel_id) {
    if (stat_list_per_channel.size() < channel_id + 1) {
      std::vector<StatBase*> s;
      stat_list_per_channel.push_back(s);
    }
    assert(channel_id < stat_list_per_channel.size());
    stat_list_per_channel[channel_id].push_back(stat);
  }
  bool is_open() {
    return stat_output.is_open();
  }
  void output(std::string filename) {
    stat_output.open(filename.c_str(), std::ios_base::out);
    if (!stat_output.good()) {
      assert(false && "Ramulator output file path error");
    }
  }
  // void printall(std::vector<unsigned> kernel_uids,
  //                unsigned int ch) {
  //   stat_output << "kernel_uids ";
  //   for (auto iter = kernel_uids.begin(); iter != kernel_uids.end(); ++iter) {
  //     stat_output << *iter;
  //     if (iter != kernel_uids.end() - 1)
  //       stat_output << ", ";
  //   }
  //   stat_output << std::endl;
  //   stat_output << "channel_id " << ch << std::endl;
  //   std::vector<StatBase*> &cur_list = stat_list_per_channel[ch];
  //   for(off_type i = 0 ; i < cur_list.size() ; ++i) {
  //     if (!cur_list[i]) {
  //       continue;
  //     }
  //     if (cur_list[i]->is_nozero() && cur_list[i]->zero()) {
  //       continue;
  //     }
  //     if (cur_list[i]->is_display()) {
  //       cur_list[i]->prepare();
  //       cur_list[i]->print(stat_output);
  //     }
  //   }
  //   printf("HELLOWORLD\n");
  // }
  ...
  void printall(std::vector<unsigned> kernel_uids, 
                ...
                unsigned int mp_id) {
    stat_output << "channel_id " << mp_id << std::endl;
    stat_output << "kernel_uids ";
    for (auto iter = kernel_uids.begin(); iter != kernel_uids.end(); ++iter) {
      stat_output << *iter;
      if (iter != kernel_uids.end() - 1)
    ...
    stat_output << "kernel_names ";
    for (auto iter = kernel_names.begin(); iter != kernel_names.end(); ++iter) {
      stat_output << *iter;
      if (iter != kernel_names.end() - 1)
        stat_output << ", ";
    }
    stat_output << std::endl;
...
class Stat : public StatBase {
 ...
 public:
  Stat() {
    //std::cout << "Stat list added" << std::endl;
    statlist.add(selfptr());
  }
  Derived &add_stat(int channel_id) {
    statlist.add(selfptr(), channel_id);
  }
